...
{
    /*
    The main goal of these classes is to deal with the native graphics of the SNES.
    The class Transformations read SNES data to draw PNG files
    The class InvertingTransformations read byte arrays to transform them in SNES data.
      These byte arrays can be obtained from a PNG using the importBPM function of the ManageBMP class.
    */
    ...
    public class Transformations
    {
        ...
        public static Bitmap transform4b(List<byte> byteMap, int offset, int size, Color[] palette = null)
        {
                    ...
                    for (int xi = 0; xi < 128 / 8; xi++)
                    {
                        ...
                        for (int j = 0; j < 8; j++)
                        {
                            ...
                            for (int k = 0; k < 8; k++)
                            {
                                ...
                                newBitmap.SetPixel(x, y, palette[cIndex]);
        ...
        public static Bitmap transform8bM7(List<byte> byteMap, int offset, List<Color> palette = null)
        {
                    ...
                    for (int yj = 0; yj < 8; yj++)
                    {
                        for (int xj = 0; xj < 8; xj++)
                        {
                            if (i >= byteMap.Count - 1)
                                return newBitmap;
                            newBitmap.SetPixel(x, y, palette[byteMap[i++]]);
                            x++;
                        }
                        x -= 8;
                        y++;
    ...
    public class ManageBMP
    {
        ...
        public static void exportBPM(String fileName, Bitmap bitmap)
        {
            /* Displays an OpenFileDialog so the user can select a res */
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "PNG File|*.png";
